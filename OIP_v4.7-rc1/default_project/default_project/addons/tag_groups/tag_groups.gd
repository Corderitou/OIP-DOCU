...
var tag_group_enum: EditorInspectorPlugin

func _enter_tree() -> void:
	tag_group_enum = TagGroupOptionButtonPlugin.new()
	add_inspector_plugin(tag_group_enum)


func _exit_tree() -> void:
	if tag_group_enum != null:
...
class TagGroupOptionButton extends EditorProperty:
	...
	func _init() -> void:
		...
		if _comms != null and _comms.has_signal("tag_groups_registered"):
			_comms.tag_groups_registered.connect(_tag_groups_registered)
	...
	func _populate_options() -> void:
		...
		if _comms == null:
			return
		var groups: Array = _comms.get_tag_groups()
		for group in groups:
