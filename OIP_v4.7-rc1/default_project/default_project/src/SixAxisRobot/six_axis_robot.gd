...
@export_range(0.1, 3.0, 0.01, "or_greater", "suffix:m") var tool_size_x: float = 0.5:
	set(value):
		tool_size_x = maxf(value, 0.1)
		_apply_eoat_settings()
		update_gizmos()

## Tool length along the EOAT local Z axis
@export_range(0.1, 3.0, 0.01, "or_greater", "suffix:m") var tool_size_z: float = 0.5:
	set(value):
		tool_size_z = maxf(value, 0.1)
		_apply_eoat_settings()
		update_gizmos()
...
var _orientation_rect: MeshInstance3D = null
...
const IK_TARGET_NAME := "IKTarget"
const IK_TARGET_SCRIPT := "res://src/SixAxisRobot/robot_ik_target.gd"

const ORIENTATION_RECT_NAME := "OrientationRect"

var _ik_target: Node3D = null


func get_ik_target() -> Node3D:
	return _ik_target if _ik_target != null and is_instance_valid(_ik_target) else null


func _ensure_ik_target() -> void:
	if not Engine.is_editor_hint():
		return
	_ik_target = get_node_or_null(NodePath(IK_TARGET_NAME)) as Node3D
	if _ik_target == null:
		var target_script: Script = load(IK_TARGET_SCRIPT)
		if target_script == null:
			push_warning("SixAxisRobot: no se encontro robot_ik_target.gd")
			return
		_ik_target = Node3D.new()
		_ik_target.name = IK_TARGET_NAME
		_ik_target.set_script(target_script)
		add_child(_ik_target)
		_ik_target.owner = null
		_ik_target.robot = self
	_ik_target.set_notify_transform(true)
	call_deferred("_sync_ik_target")


func _sync_ik_target() -> void:
	if not Engine.is_editor_hint() or _solving_ik:
		return
	if _ik_target != null and is_instance_valid(_ik_target):
		var tip_xf := get_tool_tip_transform()
		if _ik_target.global_transform.origin.distance_to(tip_xf.origin) > 0.0005 \
				or not _ik_target.global_transform.basis.is_equal_approx(tip_xf.basis):
			_ik_target.set_notify_transform(false)
			_ik_target.global_transform = tip_xf
			_ik_target.set_notify_transform(true)


func _ensure_orientation_rect() -> void:
	if not Engine.is_editor_hint():
		return
	if not _vacuum_area:
		return
	_orientation_rect = _vacuum_area.get_node_or_null(ORIENTATION_RECT_NAME) as MeshInstance3D
	if _orientation_rect:
		return
	var box := BoxMesh.new()
	box.size = Vector3(0.55, 0.015, 0.55)
	_orientation_rect = MeshInstance3D.new()
	_orientation_rect.name = ORIENTATION_RECT_NAME
	_orientation_rect.mesh = box
	_orientation_rect.owner = null
	var mat := StandardMaterial3D.new()
	mat.transparency = BaseMaterial3D.TRANSPARENCY_ALPHA
	mat.albedo_color = Color(0.2, 0.7, 1.0, 0.35)
	mat.shading_mode = BaseMaterial3D.SHADING_MODE_UNSHADED
	mat.no_depth_test = true
	mat.render_priority = 5
	_orientation_rect.material_override = mat
	_vacuum_area.add_child(_orientation_rect)
...
func _validate_property(property: Dictionary) -> void:
	...
	if property.name == "selected_waypoint":
		...
		var sorted_keys := waypoints.keys()
		sorted_keys.sort_custom(func(a: String, b: String) -> bool:
			return int(a.get_slice(": ", 0)) < int(b.get_slice(": ", 0)))
		property.hint_string = ",".join(sorted_keys) if sorted_keys.size() > 0 else "(no waypoints)"
...
## IK de pose completa: posicion del tooltip + orientacion de la copa
## (VacuumArea). CCD por gradiente: termino de posicion via Jacobiano
## (axis x (tip-pivot)) y termino de orientacion via eje-angulo del error
## cuaternionico. El solve_ik clasico queda intacto para usos solo posicionales.
func solve_ik_pose(target_pos: Vector3, target_basis: Basis, max_iterations: int = 80, tolerance: float = 0.01, rot_tolerance_deg: float = 2.0) -> bool:
	if not _initialized:
		return false
	...
	var rot_tolerance := deg_to_rad(rot_tolerance_deg)
	var max_joint_step := deg_to_rad(8.0)
	var k_rotation := 0.4

	for iteration in range(max_iterations):
		var tip := get_tool_tip_position()
		var cur_quat := get_tool_tip_transform().basis.get_rotation_quaternion()
		var e_p := target_pos - tip
		var angle_err := absf((target_quat * cur_quat.inverse()).get_angle())
		if e_p.length() < tolerance and angle_err < rot_tolerance:
			_solving_ik = false
			update_gizmos()
			return true

		var e_dir := e_p.limit_length(0.05)
		var q_err := target_quat * cur_quat.inverse()
		var w_err := (q_err.get_axis() * q_err.get_angle()).limit_length(deg_to_rad(20.0))
		...
		for j in range(5, -1, -1):
			...
			var jacobian := axis.cross(tip - pivot.global_position)
			var delta := jacobian.dot(e_dir) + k_rotation * axis.dot(w_err)
			delta = clampf(delta, -max_joint_step, max_joint_step)

			var current_angle: float = get(JOINT_ANGLE_PROPS[j])
			var new_angle := clampf(current_angle + rad_to_deg(delta), JOINT_LIMITS_MIN[j], JOINT_LIMITS_MAX[j])
			set(JOINT_ANGLE_PROPS[j], new_angle)
			tip = get_tool_tip_position()

	_solving_ik = false
	update_gizmos()
	var final_tip := get_tool_tip_position()
	var final_quat := get_tool_tip_transform().basis.get_rotation_quaternion()
	var final_angle := absf((target_quat * final_quat.inverse()).get_angle())
	return final_tip.distance_to(target_pos) < tolerance * 10.0 and final_angle < rot_tolerance * 10.0
...
func _attach_object(obj: Node3D) -> void:
	...
	if obj is RigidBody3D:
		rigid_body = obj
		if bool(obj.get_meta("oip_vacuum_detach", false)):
			# SandwichProduct placers are independent pickup bodies. Detach their
			# transform from the moving product while keeping them in its tree so
			# the station can clean them up with the product.
...
func _execute_command() -> void:
	...
	if cmd == 0:
		...
		waypoint_names.sort_custom(func(a: String, b: String) -> bool:
			var na: int = int(a.get_slice(": ", 0))
			var nb: int = int(b.get_slice(": ", 0))
			return na < nb)
