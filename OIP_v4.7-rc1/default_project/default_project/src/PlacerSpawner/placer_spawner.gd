@tool
class_name PlacerSpawner
extends ResizableNode3D

## Alimentador de placers. Emite placers por comando del PLC
## (CMD/EXEC → DONE) o en modo automático por tasa mientras corre la simulación.

## La escena a alimentar (debe derivar de Placer).
...
## Placers emitidos por minuto en modo automático (sin PLC).
...
## Prefijo de los tags de esta estación (ej. "S1"). El tag final es
## `station_prefix + suffix`.
@export var station_prefix: String = "S1"
...
func _physics_process(delta: float) -> void:
	if _comms.is_ready():
		...
		if auto_enable:
			...
			# Auto desactivado: cancelar el placer pendiente (no emitir tras el stop).
			_pending = 0
			_auto_pending = false
			_done = true
...
func _spawn_one() -> void:
	...
	if item == null:
		push_warning("PlacerSpawner: la escena configurada no pudo instanciarse")
		_done = true
		return
	var placer := item as Placer
	if placer == null:
		push_warning("PlacerSpawner: la escena configurada no deriva de Placer")
		item.queue_free()
		_done = true
		return
	...
	placer.initial_linear_velocity = spawn_velocity
	add_child(item, true)
