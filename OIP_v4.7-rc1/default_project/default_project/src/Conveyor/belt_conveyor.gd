...
## Mirror the left-side funnel controls onto the right side (angles negated, offsets shared).
@export var mirror_guards: bool = true:
	set(value):
		if value == mirror_guards:
			return
		mirror_guards = value
		_request_rebuild()
		notify_property_list_changed()

## Yaw (deg) of the back half around the belt midpoint; positive swings the tail end outward.
@export_range(-60.0, 60.0, 0.1, "suffix:°") var left_guard_angle_back: float = 0.0:
	set(value):
		if value == left_guard_angle_back:
			return
		left_guard_angle_back = value
		_request_rebuild()

@export_range(-60.0, 60.0, 0.1, "suffix:°") var left_guard_angle_front: float = 0.0:
	set(value):
		if value == left_guard_angle_front:
			return
		left_guard_angle_front = value
		_request_rebuild()

## Slide (m) of the back half along the belt axis.
@export_range(-1.0, 1.0, 0.001, "suffix:m") var left_guard_offset_back: float = 0.0:
	set(value):
		if value == left_guard_offset_back:
			return
		left_guard_offset_back = value
		_request_rebuild()

@export_range(-1.0, 1.0, 0.001, "suffix:m") var left_guard_offset_front: float = 0.0:
	set(value):
		if value == left_guard_offset_front:
			return
		left_guard_offset_front = value
		_request_rebuild()

@export_range(-60.0, 60.0, 0.1, "suffix:°") var right_guard_angle_back: float = 0.0:
	set(value):
		if value == right_guard_angle_back:
			return
		right_guard_angle_back = value
		_request_rebuild()

@export_range(-60.0, 60.0, 0.1, "suffix:°") var right_guard_angle_front: float = 0.0:
	set(value):
		if value == right_guard_angle_front:
			return
		right_guard_angle_front = value
		_request_rebuild()

@export_range(-1.0, 1.0, 0.001, "suffix:m") var right_guard_offset_back: float = 0.0:
	set(value):
		if value == right_guard_offset_back:
			return
		right_guard_offset_back = value
		_request_rebuild()

@export_range(-1.0, 1.0, 0.001, "suffix:m") var right_guard_offset_front: float = 0.0:
	set(value):
		if value == right_guard_offset_front:
			return
		right_guard_offset_front = value
		_request_rebuild()
...
func _validate_property(property: Dictionary) -> void:
	...
	elif mirror_guards and property.name in ["right_guard_angle_back", "right_guard_angle_front",
			"right_guard_offset_back", "right_guard_offset_front"]:
		property.usage = PROPERTY_USAGE_STORAGE
...
func _reconcile_path_frame_rail(rail_name: String, side_sign: float,
	...
	var mesh: ArrayMesh = ConveyorFrameMesh.create_along_path(
			_path, height, width, side_sign, head_overhang, tail_overhang, true)
...
func _rebuild_side_guards() -> void:
	...
	var eff_right_ab: float = -left_guard_angle_back if mirror_guards else right_guard_angle_back
	var eff_right_af: float = -left_guard_angle_front if mirror_guards else right_guard_angle_front
	var eff_right_ob: float = left_guard_offset_back if mirror_guards else right_guard_offset_back
	var eff_right_of: float = left_guard_offset_front if mirror_guards else right_guard_offset_front
	var funnel_active: bool = not (
			is_zero_approx(left_guard_angle_back) and is_zero_approx(left_guard_angle_front)
			and is_zero_approx(left_guard_offset_back) and is_zero_approx(left_guard_offset_front)
			and is_zero_approx(eff_right_ab) and is_zero_approx(eff_right_af)
			and is_zero_approx(eff_right_ob) and is_zero_approx(eff_right_of))
	var mid_arc: float = _path.top_surface_length * 0.5
	var funnel_up: Vector3 = _path.sample(mid_arc).basis.y
	var funnel_left := _funnel_params(_funnel_pivot(mid_arc, half_w, wt, ft, -1.0), funnel_up,
			left_guard_angle_back, left_guard_angle_front,
			left_guard_offset_back, left_guard_offset_front, 1.0, funnel_active)
	var funnel_right := _funnel_params(_funnel_pivot(mid_arc, half_w, wt, ft, 1.0), funnel_up,
			eff_right_ab, eff_right_af, eff_right_ob, eff_right_of, -1.0, funnel_active)
	for i in range(_path.runs.size()):
		...
		if left_side_guards_enabled:
			...
			for k in range(subs.size()):
				...
				_emit_side_guard_pieces("SideGuardLeft_%d_%d" % [i, k],
						sub.x, sub.y, mid_arc, run, run_basis, run_basis,
						ft_axis, z_axis * (-half_w - wt), funnel_left, funnel_active, keep)
		if right_side_guards_enabled:
			var subs: Array[Vector2] = _subdivide_arc_range_around_openings(
					run_arc_back, run_arc_front, right_openings)
			for k in range(subs.size()):
				var sub: Vector2 = subs[k]
				_emit_side_guard_pieces("SideGuardRight_%d_%d" % [i, k],
						sub.x, sub.y, mid_arc, run, run_basis, right_basis,
						ft_axis, z_axis * (half_w + wt), funnel_right, funnel_active, keep)
...
func _emit_run_guard(guard_name: String, arc_back: float, arc_front: float,
		...
		flange_offset: Vector3, lateral_offset: Vector3,
		half: String, funnel: Dictionary) -> void:
	...
	var basis: Basis = guard_basis
	if funnel.active:
		var half_sign: float = -1.0 if half == "back" else 1.0
		var yaw_rad: float = 0.0
		var slide: float = 0.0
		if half == "back":
			yaw_rad = deg_to_rad(funnel.angle_back as float) * half_sign * (funnel.side_sign as float)
			slide = funnel.offset_back as float
		else:
			yaw_rad = deg_to_rad(funnel.angle_front as float) * half_sign * (funnel.side_sign as float)
			slide = funnel.offset_front as float
		if not is_zero_approx(yaw_rad) or not is_zero_approx(slide):
			var yaw_basis: Basis = Basis(funnel.up as Vector3, yaw_rad)
			origin = ((funnel.pivot as Vector3)
					+ yaw_basis * (origin - (funnel.pivot as Vector3))
					+ run_basis * Vector3(slide, 0.0, 0.0))
			basis = yaw_basis * guard_basis
	_reconcile_side_guard(guard_name, sub_len, basis, origin)
...
func _emit_side_guard_pieces(base_name: String, arc_back: float, arc_front: float,
		mid_arc: float, run: BeltPath.Run, run_basis: Basis, guard_basis: Basis,
		flange_offset: Vector3, lateral_offset: Vector3,
		funnel: Dictionary, funnel_active: bool, keep: PackedStringArray) -> void:
	var pieces: Array[Dictionary] = [{"suffix": "", "arc_back": arc_back, "arc_front": arc_front, "half": "back"}]
	if funnel_active:
		pieces = _funnel_split(arc_back, arc_front, mid_arc)
	for p: Dictionary in pieces:
		var n: String = base_name + (p["suffix"] as String)
		keep.append(n)
		_emit_run_guard(n, p["arc_back"] as float, p["arc_front"] as float, run,
				run_basis, guard_basis, flange_offset, lateral_offset,
				p["half"] as String, funnel)


## Splits a guard's arc range at the funnel throat (belt midpoint). Pieces fully on one
## side keep the base name; only a range straddling the throat gets _B/_F suffixes.
func _funnel_split(arc_back: float, arc_front: float, mid_arc: float) -> Array[Dictionary]:
	if arc_front <= mid_arc + 0.001:
		return [{"suffix": "", "arc_back": arc_back, "arc_front": arc_front, "half": "back"}]
	if arc_back >= mid_arc - 0.001:
		return [{"suffix": "", "arc_back": arc_back, "arc_front": arc_front, "half": "front"}]
	var lo: float = arc_back + _MIN_RUN_LENGTH
	var hi: float = arc_front - _MIN_RUN_LENGTH
	if hi <= lo:
		var h: String = "front" if arc_back >= mid_arc - 0.001 else "back"
		return [{"suffix": "", "arc_back": arc_back, "arc_front": arc_front, "half": h}]
	var mid: float = clampf(mid_arc, lo, hi)
	return [
		{"suffix": "_B", "arc_back": arc_back, "arc_front": mid, "half": "back"},
		{"suffix": "_F", "arc_back": mid, "arc_front": arc_front, "half": "front"},
	]


## World-space hinge point for the funnel halves, sitting on the belt centerline at the
## throat arc, offset to the guard's lateral position. side_sign -1 = left, +1 = right.
func _funnel_pivot(mid_arc: float, half_w: float, wt: float, ft: float, side_sign: float) -> Vector3:
	var xf: Transform3D = _path.sample(mid_arc)
	return xf.origin + xf.basis.y * ft + xf.basis.z * (side_sign * (half_w + wt))


func _funnel_params(pivot: Vector3, up: Vector3, angle_back: float, angle_front: float,
		offset_back: float, offset_front: float, side_sign: float, active: bool) -> Dictionary:
	return {
		"active": active,
		"pivot": pivot,
		"up": up,
