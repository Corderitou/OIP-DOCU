@tool
class_name RollerSpurConveyorAssemblyBase
...
func _enter_tree() -> void:
	super._enter_tree()
	if not CommsHub.enable_comms_changed.is_connected(notify_property_list_changed):
		CommsHub.enable_comms_changed.connect(notify_property_list_changed)
...
func _exit_tree() -> void:
	if CommsHub.enable_comms_changed.is_connected(notify_property_list_changed):
		CommsHub.enable_comms_changed.disconnect(notify_property_list_changed)
...
func _validate_property(property: Dictionary) -> void:
	...
	if property[&"name"] == CONVEYOR_SCRIPT_FILENAME \
		...
		property[&"usage"] = PROPERTY_USAGE_CATEGORY if CommsHub.get_enable_comms() else PROPERTY_USAGE_NONE
	elif property[&"name"] == "enable_comms":
		property[&"usage"] = PROPERTY_USAGE_DEFAULT if CommsHub.get_enable_comms() else PROPERTY_USAGE_STORAGE
	elif property[&"name"] == "speed_tag_group_name":
		property[&"usage"] = PROPERTY_USAGE_STORAGE
	elif property[&"name"] == "speed_tag_groups":
		property[&"usage"] = PROPERTY_USAGE_EDITOR | PROPERTY_USAGE_NO_INSTANCE_STATE if CommsHub.get_enable_comms() else PROPERTY_USAGE_NONE
...
func _apply_spur_clipping() -> void:
	...
	if rollers_node:
		for child in rollers_node.get_children():
			if child is Roller:
				var x_spur: float = %Conveyor.position.x + rollers_node.position.x + child.position.x
...
func _update_collision_shape() -> void:
	...
	var shape_body := %Conveyor.get_node_or_null("SimpleConveyorShape") as StaticBody3D
	if not shape_body:
		return

	var collision := shape_body.get_node_or_null("CollisionShape3D") as CollisionShape3D
	if not collision:
		return
	...
	var ds_left_x := size.x / 2.0 - half_w * tan_ds - _conveyor_x_offset
	var ds_right_x := size.x / 2.0 + half_w * tan_ds - _conveyor_x_offset
	var us_right_x := -size.x / 2.0 + half_w * tan_us - _conveyor_x_offset
	var us_left_x := -size.x / 2.0 - half_w * tan_us - _conveyor_x_offset
	...
	if collision.shape is ConvexPolygonShape3D:
		convex_shape = collision.shape as ConvexPolygonShape3D
	else:
		convex_shape = ConvexPolygonShape3D.new()
