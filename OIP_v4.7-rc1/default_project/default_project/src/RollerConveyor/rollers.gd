...
const LOCAL_ROLLERS_START_OFFSET: float = 0.2
...
func _add_or_remove_rollers(conveyor_length: float) -> void:
	var available_length := conveyor_length - LOCAL_ROLLERS_START_OFFSET - ROLLERS_DISTANCE
...
func _apply_roller_length(roller: Roller) -> void:
	...
	var roller_conveyor_x := -_length / 2.0 + LOCAL_ROLLERS_START_OFFSET + base_x
